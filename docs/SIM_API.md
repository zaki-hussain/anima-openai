...
- 61 seeded discharge letters (`document-batch-*`) across many patients: 37 sent, 12 reviewed, 8 filed, 4 hospital-only drafts. 13 open GP tasks "Confirm follow-up arrangements". **Most cohort patients therefore already have an old `sent` letter in the GP inbox** — the engine must only count records *we* created: `provenance.created.actor.kind === 'team'` (and `.name` = our world's team name) or `createdAt >= fitAt`. Seeded records are `actor.kind === 'simulation'`.
- After `POST /api/clock` advances, the sim's own agents also act: new A&E arrivals (`emergency.arrived`), synthetic patient requests, `flow.pressure` ("Demand exceeds staffed bed availability") — useful colour for the story, and a reason our freed beds matter.
...
- World creation: slow (minutes). Reads: ~1 s (the unfiltered GP view is slow and huge — never call it in a loop). **Actions and clock advances: ~3 s each when the instance is healthy (measured 15:00 BST after a restart: create_task 3.0 s, advance 2.8 s); under load they degraded to 15–22 s and then the instance returned 502s for a few minutes.** Budget for the slow case in demo loops; parallelise across patients where safe (different resources), never race on the same resource version.
