...
- Sim writes are ~1–3 s each; reads are fast except the unfiltered GP view (never call `/api/sites/gp/view` without `?patient=`). Actions on existing records need `resourceId` + `expectedVersion`; always send an `Idempotency-Key`; on 409 re-read and retry once.
